...
impl SIMDSimpleRandom {
    ...
    pub fn next_int(&mut self) -> v128 {
        //supposedly i32x4
        //self.seed = (Wrapping(self.seed) * Wrapping(MULT) + Wrapping(0xB)).0 & MASK;
        //(self.seed as u64 >> 17) as i32
        self.seeds = self
            .seeds
            .map(|seed| v128_and(i64x2_add(i64x2_mul(seed, MULTx2), i64x2_splat(0xB)), MASKx2));
        let unsigned: [v128; 2] = self.seeds.map(|seed| u64x2_shr(seed, 17));
        i32x4(
            i64x2_extract_lane::<0>(unsigned[0]) as i32,
            i64x2_extract_lane::<1>(unsigned[0]) as i32,
            i64x2_extract_lane::<0>(unsigned[1]) as i32,
            i64x2_extract_lane::<1>(unsigned[1]) as i32,
        )
    }
    ...
    pub fn next_int_bound(&mut self, bound_num: i32) -> v128 {
        //supposedly i32x4
        /*let mut r = Wrapping(self.next_int());
        let m = bound - Wrapping(1);
        if (bound & m) == Wrapping(0) {
        ...
        if (bound_num & m_num) == 0 {
            ...
            return i32x4(
                i64x2_extract_lane::<0>(low) as i32,
                i64x2_extract_lane::<1>(low) as i32,
                i64x2_extract_lane::<0>(high) as i32,
                i64x2_extract_lane::<1>(high) as i32,
            );
        }
        ...
        loop {
            r = i32x4_sub(
                u,
                i32x4_mul(
                    i32x4_trunc_sat_f32x4(f32x4_div(f32x4_convert_i32x4(u), boundf)),
                    bound,
                ),
            );
    ...
    fn generic_enchantibility(&mut self, shelves: i32) -> v128 {
        ...
        //first + 1 + (shelves >> 1) + second
        i32x4_add(
            i32x4_add(first, i32x4_splat(1)),
            i32x4_add(i32x4_splat(shelves >> 1), second),
        )
    }
    ...
    fn levels_slot2(&mut self, shelves: i32) -> v128 {
        //(self.generic_enchantibility(shelves) * 2 / 3) + 1
        i32x4_add(
            SIMDSimpleRandom::divide_by_3(i32x4_mul(
                self.generic_enchantibility(shelves),
                i32x4_splat(2),
            )),
            i32x4_splat(1),
        )
